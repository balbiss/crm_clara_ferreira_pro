...
**Tipos de nó** (`FlowNode::NODE_TYPES`): `trigger` (Novo contato/Palavra-chave/Mensagem recebida/Evento/Webhook/Manual em `data.trigger_type`), `send_message`, `ask_question` (guarda resposta em `data.variable`), `send_media` (subtipos via `data.media_type`: image/audio/document — `video` continua existindo no schema mas foi tirado da paleta e do dropdown do frontend, ver nota abaixo), `options` (Botões e Lista de opções via `data.mode`, saídas dinâmicas nomeadas por opção — mesma ideia do `condition`), `condition` (`data.check_type`: variável/resposta/horário/dia/etiqueta/status — mas só variável é avaliado de verdade hoje), `wait`, `action` (add_tag/remove_tag/assign_agent/update_variable/send_webhook via `data.action_type` — mesmo vocabulário de `PipelineTrigger#action_type`, pensado pra um dia ser executado pelo mesmo tipo de runner), `end`.

**Vídeo removido da paleta (2026-08-31)**: mandar vídeo pela WAHA no engine WEBJS dá 422 ("Use devlikeapro/waha:chrome"). Tentei trocar a imagem do container da WAHA pra `:chrome` (confirmado grátis/público, sem licença) — o Chrome real crashava ao abrir dentro do container mesmo depois de `--shm-size=1gb` e `WAHA_WEBJS_PUPPETER_ARGS=--no-sandbox --disable-dev-shm-usage --disable-gpu` (erro `TargetCloseError: Protocol error (Target.setAutoAttach): Target closed`, sintoma clássico de Chrome não conseguindo subir em Docker restrito). Revertido pra `devlikeapro/waha:latest` (Chromium) — a sessão reconectou sozinha, sem precisar escanear QR de novo. Fica como limitação conhecida/aceita por enquanto; imagem/áudio/documento continuam funcionando normalmente.

**Flow#inbox_id (2026-08-31)**: o gatilho por palavra-chave só escuta a caixa escolhida no fluxo (`inbox.flows`, não `account.flows`) — achado pelo próprio usuário testando ("não tem como escolher qual caixa"). Fluxo sem caixa definida não dispara em lugar nenhum, de propósito. Seletor em `/fluxos/novo` e no topo do editor.

**Execução ao vivo (`FlowRunnerService`)**: gatilho por Palavra-chave dispara de verdade (hookado no webhook da WAHA, antes do bloco de IA). `send_message`/`send_media`/`wait`/`condition`/`action`/`end` executam de verdade. `ask_question` e `options` (Botões/Lista) PARAM a execução e esperam a próxima mensagem — estado guardado em `FlowRun` (novo model: `flow_id`/`conversation_id`/`contact_id`/`current_node_key`/`variables` jsonb/`status`). `FlowRunnerService.continue_with_reply` é chamado ANTES do gatilho por palavra-chave pra toda mensagem recebida: se a conversa tem um `FlowRun` em `waiting_reply`, a mensagem vira a variável (Perguntar) ou é casada contra as opções por número/texto (Botões/Lista — cuidado com a direção do `include?`, ver código). Se a resposta de Botões não bate com nenhuma opção, o fluxo pede pra responder de novo e CONTINUA esperando (não encerra sozinho — bug real encontrado num teste ao vivo). `condition` avalia de verdade contra `FlowRun#variables`. `action` cobre add_tag/remove_tag (na CONVERSA — `conversation_tags`, não no contato — é onde a tela de conversa mostra "Adicionar etiqueta" de verdade, com broadcast ActionCable pra atualizar sem recarregar)/assign_agent/update_variable/send_webhook, mesmo vocabulário de `PipelineTrigger#action_type`. `wait` enfileira `FlowContinueJob` com `.set(wait: segundos)` (SolidQueue, mesmo padrão de `ReguaAutoAdvanceJob`) e para a execução síncrona ali. `send_media`: upload direto do computador (`FlowNode#media`, ActiveStorage, endpoint `POST /flows/:flow_id/nodes/:key/media`) é o caminho confiável — reaproveita `WhatsappWahaService#send_message` com anexo; URL externa (`WhatsappWahaService#send_media_by_url`) existe como alternativa mas deu 500 num teste real (não confiável nessa instância WAHA). Todos testados ponta a ponta com WhatsApp real em 2026-08-31.
...
**Pendente do prompt original**: rascunho/publicado separado (só existe Ativo/Inativo), simulador "Testar fluxo", templates prontos, gatilhos além de Palavra-chave (novo contato/mensagem recebida/evento/webhook/manual), canvas mobile.

### 10.4 Cor de marca (2026-08-31)

`--primary`/`--primary-hover` em `style.scss` eram `#d49ba7`/`#ba5e72` (rosa apagado) — lia como "botão desativado" (reclamação real do usuário sobre o CRM inteiro, não só Fluxos). Trocado pra `#ff007f`/`#cc0066`, que já era a cor "oficial" documentada em `config/brand.js` mas nunca tinha sido conectada a nada. Como a maioria dos componentes usa `var(--primary, #d49ba7)` (fallback hardcoded, não só a variável), foi preciso trocar o hex também em ~43 arquivos `.vue`/`.js` (find-replace em massa, incluindo variantes `rgba()` dos mesmos tons usadas em sombra/foco) — não bastava mudar só o `:root`.
